window.addEventListener('load', () => {
  ...
  if (menuIcon && submenu) {
    ...
    function openMenu() {
      ...
      if (overlay) { overlay.classList.add('active'); overlay.removeAttribute('hidden'); }
    ...
    menuIcon.addEventListener("click", (e) => {
      e.stopPropagation();
      if (submenu.classList.contains('active')) closeMenu();
      else openMenu();
    });

    // Click fuera del menú para cerrarlo (overlay)
    if (overlay) {
    ...
    links.forEach((link, idx) => {
      ...
      link.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowDown') { e.preventDefault(); const next = links[idx+1] || links[0]; next.focus(); }
        if (e.key === 'ArrowUp') { e.preventDefault(); const prev = links[idx-1] || links[links.length-1]; prev.focus(); }
        if (e.key === 'Enter') { link.click(); }
      });

      // mouse hover focuses link (useful on TV with mouse)
...
const PELICULAS_POPULARES = [
  {
  ...
  },
   {
    ...
    category: "terror",
    img: "https://lamovie.org/wp-content/uploads/thumbs/c22fdaaf2da7157fd0ecc757f34ff726_hd.webp",
    desc: "La joven hija de un periodista desaparece en el desierto. Ocho años después, la familia rota se sorprende cuando ella regresa con ellos, mientras lo que debería ser[...],
    video: "https://vimeos.net/embed-rm46q7vy3t52.html",
    icon: "🔥"
  },
  {
